...
function Onboarding() {
  ...
  const finalizar = async () => {
    if (!user) return;
    if (!nome.trim()) { setErro("Informe o nome da sua produtora."); return; }
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="mb-6 flex gap-2">
          {[1, 2].map(s => (
            <div key={s} className={cn("h-1 flex-1 rounded-full transition-all", s <= step ? "bg-primary" : "bg-surface-2")} />
          ))}
        </div>
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <label className={cn("flex h-20 cursor-pointer items-center justify-center rounded-xl border-2 border-dashed transition",
                  ...
                  {logoPreview
                    ...
                    : <span className="text-xs text-muted-foreground">Clique para enviar · PNG, SVG ou JPG</span>}
                  <input type="file" accept="image/*" className="hidden" onChange={handleLogo} />
                </label>
                {logoPreview && (
                  <button onClick={() => { setLogoFile(null); setLogoPreview(null); }}
                    className="text-[11px] text-muted-foreground hover:text-destructive">Remover logo</button>
                )}
              </div>

              {erro && <p className="text-xs text-destructive">{erro}</p>}

              <Button className="w-full" onClick={() => { if (!nome.trim()) { setErro("Informe o nome."); return; } setErro(null); setStep(2); }}>
                Próximo →
              </Button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">Escolha a cor de destaque da interface</p>
              </div>

              <div className="grid grid-cols-3 gap-2.5">
                {CORES.map(c => (
                  <button key={c.value} onClick={() => { setCor(c.value); applyBrandColor(c.value); }}
                    className={cn("flex flex-col items-center gap-2 rounded-xl border p-3 transition",
                      cor === c.value ? "border-white/30 bg-surface-2" : "border-border/40 bg-surface-1/40 hover:border-border")}>
              ...
              <div className="rounded-xl border border-border/40 bg-surface-2/30 p-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="grid h-7 w-7 place-items-center rounded-lg text-xs font-bold text-primary-foreground"
                    style={{ backgroundColor: cor, boxShadow: `0 0 16px -2px ${cor}` }}>M</div>
                  <span className="text-sm font-medium">{nome || "Sua Produtora"}</span>
                  <div className="ml-auto rounded-md px-2.5 py-1 text-xs font-medium text-primary-foreground"
                    style={{ backgroundColor: cor }}>+ Nova tarefa</div>
                </div>
              </div>
