...
import { CATEGORIAS, type CategoriaRecurso } from "@/lib/mock/biblioteca";
...
export function CategoriaCard({
  ...
  const meta =
    categoria === "favoritos"      ? { label: "Favoritos",      icone: "⭐", descricao: "Seus modelos mais usados." } :
    categoria === "compartilhados" ? { label: "Compartilhados", icone: "👥", descricao: "Visíveis para sua equipe." } :
    categoria === "marketplace"    ? { label: "Marketplace",    icone: "🛒", descricao: "Modelos prontos da comunidade." } :
  ...
  const inner = (
    <div className={`group relative flex h-full flex-col gap-3 overflow-hidden rounded-2xl border p-5 transition-all duration-300 ${disabled
      ...
      <div className="grid size-11 place-items-center rounded-xl bg-primary/10 text-2xl ring-1 ring-primary/20">{meta.icone}</div>
      <div>
