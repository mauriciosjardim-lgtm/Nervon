    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-md gap-5 p-7">
        ...
        <div className="space-y-4">
          <div className="space-y-2"><Label className="text-xs">Título</Label><Input value={titulo} onChange={e => setTitulo(e.target.value)} placeholder="Ex: Entrega final" /></div>
          <div className="space-y-2"><Label className="text-xs">Data</Label><Input type="datetime-local" value={data} onChange={e => setData(e.target.value)} /></div>
          <p className="rounded-md border border-primary/30 bg-primary/5 px-3 py-2 text-[11px] text-primary">🚩 Marcos aparecem automaticamente na Agenda</p>
        </div>
