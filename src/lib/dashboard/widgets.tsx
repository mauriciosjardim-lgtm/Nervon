...
const ReceitaMes = () => {
  ...
  const hoje = new Date();
  const faturamento = lancs
    .filter(l => l.tipo === "receita" && l.pagamentoEm && isSameMonth(new Date(l.pagamentoEm), hoje))
    .reduce((s, l) => s + l.valor, 0);
  return <Stat icon={Wallet} label="Receita do mês" value={brl(faturamento)} tone="positive" />;
};
...
const LucroMes = () => {
  ...
  const receitas = lancs
    ...
    .reduce((s, l) => s + l.valor, 0);
  const custos = lancs
    .filter(l => l.tipo === "despesa" && l.pagamentoEm && isSameMonth(new Date(l.pagamentoEm), hoje))
    .reduce((s, l) => s + l.valor, 0);
...
const FaturamentoGrafico = () => {
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      <div>
        <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Faturamento × Custos</div>
      ...
      <div className="min-h-0 flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
            ...
            <XAxis dataKey="semana" tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `${(v / 1000).toFixed(0)}k`} />
            <Tooltip contentStyle={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 10, fontSize: 12 }} formatter={(v: number) => brl(v)} />
            <Area type="monotone" dataKey="faturamento" stroke="var(--primary)" strokeWidth={2} fill="url(#gFat)" />
            <Area type="monotone" dataKey="custos" stroke="var(--muted-foreground)" strokeWidth={1.5} fill="transparent" strokeDasharray="4 4" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
...
const IndicadoresFinanceiros = () => {
  const lancs = useLancamentos();
  const hoje = new Date();
  const faturamento = lancs.filter(l => l.tipo === "receita" && l.pagamentoEm && isSameMonth(new Date(l.pagamentoEm), hoje)).reduce((s, l) => s + l.valor, 0);
  const custos = lancs.filter(l => l.tipo === "despesa" && l.pagamentoEm && isSameMonth(new Date(l.pagamentoEm), hoje)).reduce((s, l) => s + l.valor, 0);
...
// -- registry ------------------------------------------------------------------
export const widgetRegistry: Record<string, WidgetMeta & { component: React.ComponentType }> = {
  "receita-mes":              { type: "receita-mes",              title: "Receita do mês",         description: "Total faturado no mês corrente",    category: "Financeiro",    defaultSize: "sm", icon: Wallet,         component: ReceitaMes },
  "lucro-mes":                { type: "lucro-mes",                title: "Lucro do mês",           description: "Lucro líquido e margem",            category: "Financeiro",    defaultSize: "sm", icon: TrendingUp,     component: LucroMes },
  "receita-recorrente":       { type: "receita-recorrente",       title: "A receber (previsto)",   description: "Lançamentos futuros de receita",    category: "Financeiro",    defaultSize: "sm", icon: Repeat,         component: ReceitaRecorrente },
  "contas-receber":           { type: "contas-receber",           title: "Contas a receber",       description: "Próximos recebimentos previstos",   category: "Financeiro",    defaultSize: "sm", icon: ArrowDownCircle, component: ContasReceber },
  "contas-pagar":             { type: "contas-pagar",             title: "Contas a pagar",         description: "Despesas a vencer",                 category: "Financeiro",    defaultSize: "sm", icon: ArrowUpCircle,  component: ContasPagar },
  "faturamento-grafico":      { type: "faturamento-grafico",      title: "Faturamento × Custos",   description: "Comparativo de 6 meses",            category: "Financeiro",    defaultSize: "lg", icon: TrendingUp,     component: FaturamentoGrafico },
  "indicadores-financeiros":  { type: "indicadores-financeiros",  title: "Indicadores",            description: "Visão completa do financeiro",      category: "Financeiro",    defaultSize: "md", icon: Activity,       component: IndicadoresFinanceiros },

  "pipeline":                 { type: "pipeline",                 title: "Pipeline comercial",     description: "Valor total no funil",              category: "Comercial",     defaultSize: "sm", icon: Target,         component: Pipeline },
  "leads-quentes":            { type: "leads-quentes",            title: "Leads quentes",          description: "Negociações com maior valor",       category: "Comercial",     defaultSize: "lg", icon: Sparkles,       component: LeadsQuentes },
  "propostas-aguardando":     { type: "propostas-aguardando",     title: "Propostas aguardando",   description: "Enviadas sem resposta",             category: "Comercial",     defaultSize: "md", icon: FileText,       component: PropostasAguardando },
  "clientes-ativos":          { type: "clientes-ativos",          title: "Clientes no CRM",        description: "Base de clientes vigente",          category: "Comercial",     defaultSize: "sm", icon: Users,          component: ClientesAtivos },
  "followups":                { type: "followups",                title: "Follow-ups",             description: "Próximas ações comerciais",         category: "Comercial",     defaultSize: "md", icon: Bell,           component: Followups },

  "projetos-ativos":          { type: "projetos-ativos",          title: "Projetos ativos",        description: "Em produção no momento",            category: "Operacional",   defaultSize: "sm", icon: FolderKanban,   component: ProjetosAtivos },
  "projetos-criticos":        { type: "projetos-criticos",        title: "Projetos críticos",      description: "Atrasados ou em risco",             category: "Operacional",   defaultSize: "md", icon: AlertTriangle,  component: ProjetosCriticos },
  "tarefas-pendentes":        { type: "tarefas-pendentes",        title: "Tarefas pendentes",      description: "Tudo que precisa sair",             category: "Operacional",   defaultSize: "md", icon: CheckCircle2,   component: TarefasPendentes },
  "proximas-gravacoes":       { type: "proximas-gravacoes",       title: "Próximas gravações",     description: "Captações agendadas",               category: "Operacional",   defaultSize: "md", icon: Video,          component: ProximasGravacoes },

  "meu-dia":                  { type: "meu-dia",                  title: "Meu dia",                description: "Compromissos e tarefas de hoje",    category: "Pessoal",       defaultSize: "md", icon: CalendarIcon,   component: MeuDia },
  "agenda":                   { type: "agenda",                   title: "Agenda",                 description: "Próximos compromissos",             category: "Pessoal",       defaultSize: "md", icon: CalendarIcon,   component: Agenda },
